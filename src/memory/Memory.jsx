...
const checkBoard = (board, setScore) => {
  ...
  for (let row = 0; row < board.length; row++) {
    for (let col = 0; col < board[row].length; col++) {
      ...
      if (board[row][col] === WRONG || board[row][col] === MISSING)
        isCorrect = false;
    }
  }
                  ...
                  <div className="row" key={`row-${rowIdx}`}>
                    {board[rowIdx] &&
                      board[rowIdx].map((col, colIdx) => {
                        ...
                        if (col === "*" && showHighlightedCells)
                          cellClassName = "cursor-disabled highlighted";
